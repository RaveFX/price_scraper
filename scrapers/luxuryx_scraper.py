...
class LuxuryXScraper(BaseScraper):
    ...
    def scrape(self) -> List[Dict[str, Any]]:
        url = f"{self.base_url}/android-price"
        headers = {
        ...
        logger.info(f"Fetching LuxuryX Android Price page: {url}")
        try:
        ...
        logger.info(f"Found {len(matches)} productCard blocks in the HTML.")
        ...
        for i, match in enumerate(matches):
            try:
                ...
                brand = raw_prod.get("brand", "Samsung").strip()
                category = raw_prod.get("category", "Android").strip()
